...
double benchmark_create_joinsplit()
{
    ...
    struct timeval tv_start;
    timer_start(tv_start);
    JSDescription jsdesc(*psnowgemParams,
                         pubKeyHash,
                         anchor,
                         {JSInput(), JSInput()},
                         {JSOutput(), JSOutput()},
    ...
    auto verifier = libsnowgem::ProofVerifier::Strict();
    assert(jsdesc.Verify(*psnowgemParams, verifier, pubKeyHash));
    return ret;
}
...
double benchmark_verify_joinsplit(const JSDescription &joinsplit)
{
    ...
    uint256 pubKeyHash;
    auto verifier = libsnowgem::ProofVerifier::Strict();
    joinsplit.Verify(*psnowgemParams, verifier, pubKeyHash);
    return timer_stop(tv_start);
}
...
double benchmark_try_decrypt_notes(size_t nAddrs)
{
    ...
    for (int i = 0; i < nAddrs; i++) {
        auto sk = libsnowgem::SpendingKey::random();
...
public:
    FakeCoinsViewDB(std::string dbName, uint256& hash, bool fMemory, bool fWipe) : CCoinsViewDB(dbName, 100, false, false), hash(hash) {}
...
double benchmark_connectblock_slow()
{
    ...
    FakeCoinsViewDB fakeDB("benchmark/block-107134-inputs", hashPrev, false, false);
